...
//#define DEBUG // Comment out this line to disable printing
...
void *mm_malloc(size_t size) {
  ...
  PRINT("count is %d\n", count);
    ...
    PRINT("adding heap in mm_malloc\n");
      /* create heap and set header and footer of heap*/
    void * heap; 
    ...
    if ((bp = find_fit(newsize)) != NULL) {                     //if we can fit it anywhere in the free list
      ...
      PRINT("In mm_malloc, place Returns this pointer**************************:\n\n\n\n\n");
    ...
    PRINT("no fit found in mm_malloc after while loop 1\n");
    ...
    PRINT("no fit found in mm_malloc after while loop 2\n");
...
void add_free(void *bp){
    ...
    if(freelist_start == NULL){
        PRINT("freelist_start is NULL in add_free\n");
    ...
    //otherwise the freelist is not empty
      ...
      PRINT("freelist_start is NOT  NULL in add_free\n");
        char *temp;
    ...
    if((size = GET_SIZE(HDRP(bp))) == 0) {
     PRINT("bp has no size in add_free\n"); 
     return;
    }
    
    if (size != ALIGN(size)) {
     PRINT("Not aligned in add free\n");
     return;
    }
 ...
 */
 static void *coalesce(void *bp)
 {
   ...
   //check for memory in bounds here
    ...
    if(!inbounds(NEXT_BLKP(bp))){
      if(prev_alloc){
          return bp;
      }
      else{  //we should coalesce with the previously allocated block
	...
	//abort();
    ...
    }else{ //else the next block is in bounds and you are not at the end of the heap
      if(prev_alloc && next_alloc){
          return bp;
      }
      else if(prev_alloc && !next_alloc){ //previous allocated but next is free
      ...
      else if(!prev_alloc && next_alloc){ //previous free and next allocated
          remove_free(PREV_BLKP(bp));
          size+= GET_SIZE(HDRP(PREV_BLKP(bp)));
          PUT(HDRP(PREV_BLKP(bp)),PACK(size,0));
          PUT(FTRP(PREV_BLKP(bp)),PACK(size,0));
      ...
      else{ //previous and next are free and we need to coalsce on both sides
          remove_free(PREV_BLKP(bp));
          remove_free(NEXT_BLKP(bp));
...
// PRINT("hi\n");
//     if( (prev_alloc && !inbounds(NEXT_BLKP(bp))) || (prev_alloc && next_alloc) {
//         return bp;
//     }
 ...
 */
 void *find_fit(size_t size){
   ...
   while ((current = succ(current)) != freelist_end) {
     PRINT("find fit while loop\n");
      if (GET_SIZE(HDRP(current)) == size) {
    PRINT("in while loop of find_fit()\n");
    remove_free(current);
////printblock(current);
    ...
    // split block
          size_t sizeDiff;
         PRINT("in find_fit(), split block bp %x with size %d\n", current, size);
...
PRINT("enter place\n");  
  ...
  if (bp == NULL || !inbounds(bp))
   ...
   // check size is aligned;
   if (size != ALIGN(size)) {
     PRINT("in place(), size %d is not aligned!\n", size);
     ////printblock(bp);
     size = ALIGN(size);
   }
  
  if (GET_SIZE(HDRP(bp)) < size) {
     PRINT("in place(), size should have been determined by find_fit()\n", size);
    // //printblock(bp);
     return; //block is no longer large enough to accomodate for the size
  }
  
  if (GET_SIZE(HDRP(bp)) == size) {
    PRINT("in place(), perfect fit with size %d\n", size);
    //remove_free(bp);          // remove block from free list
    PRINT("perfect fit after remove free\n");
   // //printblock(bp);
  ...
  if (GET_SIZE(HDRP(bp)) > size) {
    PRINT("Should not enter here!!! find_fit should give perfect fitting block\n");
    abort();
   //PRINT("in place(), split block bp %x with size %d\n", bp, size);
...
/* allocAddress points to 1 word after the header */
    ...
    PRINT("new bp address is %x and sizeDiff is %d\n", bp, sizeDiff);
    */
    return;
  }
...
void printblock(void *bp, char * msg){
  ...
  PRINT("bp header addres is %x\n bp footer address is %x\n bp address is %x\n", HDRP(bp),FTRP(bp),bp);  
  unsigned int difference = FTRP(bp) + (~((unsigned int) bp)+1); 
  PRINT("size without header and footer is %d\n", difference);
  
  if(headerAlloc == 0) {
    PRINT("succ address %p,\npred address: %p\n", succ(bp), pred(bp));
  }
  
  PRINT("--------------------end //printblock\n");
  return;
 }
 ...
 /*
  * /u/cs252/labs15/malloctraces/
  * amptjp-bal.rep   cccp.rep            random2-bal.rep   realloc.rep
amptjp.rep       coalescing-bal.rep  random2.rep       short1-bal.rep
binary2-bal.rep  coalescing.rep      random-bal.rep    short1.rep
binary2.rep      cp-decl-bal.rep     random.rep        short2-bal.rep
binary-bal.rep   cp-decl.rep         realloc2-bal.rep  short2.rep
binary.rep       expr-bal.rep        realloc2.rep
cccp-bal.rep     expr.rep            realloc-bal.rep
*/
